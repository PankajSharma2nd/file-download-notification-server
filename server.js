...
// Endpoint to handle download completion and send admin notification
app.post('/api/download-complete', async (req, res) => {
  try {
    ...
    try {
      ...
      return res.status(500).json({
        success: false,
        message: `Error sending admin notification: ${error.message}`
      });
    }
  } catch (error) {
    console.error('Error in download-complete handler:', error);
    return res.status(500).json({
      success: false,
      message: `Server error: ${error.message}`
    });
  ...
  try {
    ...
    const payload = JSON.stringify({
      ...
      data: {
        ...
        needsAdminNotification: true // Flag to indicate admin notification is needed
      }
    });
...
async function sendAdminNotification(subscription) {
  try {
    // Prepare notification payload
    const payload = JSON.stringify({
    ...
    // Send the notification
    await webpush.sendNotification(subscription, payload);
    console.log('Sent admin notification reminder');
    return true;
  } catch (error) {
    console.error('Error sending admin notification:', error);
